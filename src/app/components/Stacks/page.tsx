      ...
      <div className="min-h-screen bg-[#010C04] text-white">
        <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 md:px-8 py-12 sm:py-16 xl:mt-[-40px] mt-[-20px] md:mt-[-100px] lg:mt-[-30px]">
          <div className="flex flex-col justify-center space-y-8 sm:space-y-12">
            {/* Header */}
            <h2 className="text-[28px] sm:text-[32px] md:text-[35px] font-bold text-[#53C218] text-center mt-[20px] sm:mt-[30px]">
              Stacks
            </h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8 lg:gap-16 w-full">
              {skills.map((skill, index) => (
                ...
                >
                  <skill.Icon
                    className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 mb-3 transition-transform duration-300 group-hover:scale-110"
                    style={{ color: skill.color }}
                  />
                  <span className="text-[15px] sm:text-[17px] md:text-[19px] font-medium text-gray-300 group-hover:text-white text-center">
                    {skill.name}
                  </span>
                </div>
              ))}
